...
###################################################################################
###################################################################################
#BREAKPOINT DETECTION USING THE STRUCCHANGE PACKAGE

#------------------------------------------------------------------------------------
#first I'm going to generate a new set of fake data that has 6 years worth of 
# monthly data (12*6=72 observations) before a policy change and 2 years of monthly
# (2*12=24 observations) data following the policy change

# the series will be characterized by a distinct seasonal pattern before the policy
# and notable lack of seasonality after the change
...
m <- c(100,200,300,400)
sig <- c(5,10,15,20)
y <- c(c(rnorm(1,m[2],sig[2]),rnorm(4,m[1],sig[1]),rnorm(1,m[2],sig[2]),rnorm(2,m[4],sig[4]),rnorm(3,m[1],sig[1]),rnorm(1,m[2],sig[2])),
       c(rnorm(1,m[2],sig[2]),rnorm(4,m[1],sig[1]),rnorm(1,m[2],sig[2]),rnorm(2,m[4],sig[4]),rnorm(3,m[1],sig[1]),rnorm(1,m[2],sig[2])),
       c(rnorm(1,m[2],sig[2]),rnorm(4,m[1],sig[1]),rnorm(1,m[2],sig[2]),rnorm(2,m[4],sig[4]),rnorm(3,m[1],sig[1]),rnorm(1,m[2],sig[2])),
       c(rnorm(1,m[2],sig[2]),rnorm(4,m[1],sig[1]),rnorm(1,m[2],sig[2]),rnorm(2,m[4],sig[4]),rnorm(3,m[1],sig[1]),rnorm(1,m[2],sig[2])),
       c(rnorm(1,m[2],sig[2]),rnorm(4,m[1],sig[1]),rnorm(1,m[2],sig[2]),rnorm(2,m[4],sig[4]),rnorm(3,m[1],sig[1]),rnorm(1,m[2],sig[2])),
       c(rnorm(1,m[2],sig[2]),rnorm(4,m[1],sig[1]),rnorm(1,m[2],sig[2]),rnorm(2,m[4],sig[4]),rnorm(3,m[1],sig[1]),rnorm(1,m[2],sig[2]))
...
df <- data.frame(year=year,month=month,days=y) 

df2 <- data.frame(year=c(rep(2011,12),rep(2012,12)),month=c(rep(1:12,2)),
                  days=rnorm(24,150,7))


z <- tbl_df(rbind(df,df2)) %>% mutate(t=seq(1:96))

ggplot(z,aes(x=t,y=days)) + geom_line() + theme_economist() + 
  theme(axis.text.y=element_blank())                                                                                                           se = F, method = "lm", formula = y ~ poly(x, 8))
#-------------------------------------------------------------------
...
#Try to identify this break with the MSwM package
z <- z %>% mutate(jan=ifelse(month==1,1,0),
...
summary(lm(days~factor(month),data=z))

#MRSS with no autoregressive term---------------------------------
mod1 <- lm(days~feb+mar+apr+may+june+july+aug+sept+oct+nov+dec,data=z)
...
#MRSS with AR(1) term---------------------------------
mod2 <- lm(days~feb+mar+apr+may+june+july+aug+sept+oct+nov+dec,data=z)
